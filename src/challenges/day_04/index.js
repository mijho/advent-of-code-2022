...
export const findConflictingTasks = async (part, input) => {
  ...
  for await (const line of lr) {
    ...
    if (
      (leMin <= reMin && leMax >= reMax) ||
      (reMin <= leMin && reMax >= leMax)
    ) {
      total++;
    } else if (part === 2) {
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  console.log(
    `The total score if following the guide Part 1: ${await findConflictingTasks(
      1,
      "./input.txt"
    )}`
  );
  console.log(
    `The total score if following the guide Part 2: ${await findConflictingTasks(
      2,
      "./input.txt"
    )}`
  );
